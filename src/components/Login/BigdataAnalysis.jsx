...
const DateRangeAnalysis = () => {
  ...
  const fetchData = async (start, end) => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/date-range-analysis`,
        {
          params: {
            startDate: format(start, "yyyy-MM-dd"),
            endDate: format(end, "yyyy-MM-dd"),
  ...
  const fetchUnknownData = async (start, end) => {
    try {
      ...
      setCurrentImages(response.data.image_paths);
    } catch (error) {
      console.error("Error fetching unknown data:", error);
    }
  };

  const handleDatesData = () => {
    if (startDate && endDate) {
      fetchData(startDate, endDate);
      fetchUnknownData(startDate, endDate);
    }
  ...
  const handleNextImage = () => {
    if (currentImageIndex + 5 < currentImages.length) {
      setCurrentImageIndex((prevIndex) => prevIndex + 5);
    }
  };
  ...
  return (
    <div className="p-4">
      <h2 className="text-lg font-bold mb-4">Date Range Analysis</h2>
      <div className="flex gap-4 mb-4">
        ...
        <DatePicker
          ...
          className="border p-2"
        />
        <button onClick={handleDatesData} className="bg-blue-500 text-white px-4 py-2 rounded">
          Fetch Data
        </button>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2 overflow-x-auto">
          {currentImages.slice(currentImageIndex, currentImageIndex + 5).map((img, index) => (
            <img key={index} src={img} alt="Fetched" className="w-32 h-32 object-cover" />
          ))}
        </div>
        <div className="flex gap-4 mt-2">
          <button onClick={handlePrevImage} disabled={currentImageIndex === 0} className="bg-gray-300 px-3 py-1 rounded">
            Prev
          </button>
          <button onClick={handleNextImage} disabled={currentImageIndex + 5 >= currentImages.length} className="bg-gray-300 px-3 py-1 rounded">
            Next
          </button>
        </div>
      </div>
